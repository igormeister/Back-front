...
export class ClientService {
  ...
  async findOne(username: string): Promise<Client> {
    return await this.ClientModel.findOne({ user_login: username });
  }
...
export class AuctionService {
  ...
  async getAuction(AuctionID: string): Promise<Auction> {
    const Auction = await this.AuctionModel.findOne({ _id: AuctionID }).exec();
    return Auction;
  }
  ...
  async editAuction(AuctionID: string, AuctionDto: AuctionDto): Promise<Auction> {
    const editedAuction = await this.AuctionModel.updateOne({ _id: AuctionID }, AuctionDto, { new: true });
    return editedAuction;
  }

  async deleteAuction(AuctionID: string): Promise<Auction> {
    const deletedAuction = await this.AuctionModel.deleteOne({ _id: AuctionID });
...
export class LotService {
  ...
  async getLot(LotID: string): Promise<Lot> {
    const Lot = await this.LotModel.findOne({ _id: LotID }).exec();
    return Lot;
  }
  ...
  async editLot(LotID: string, LotDto: LotDto): Promise<Lot> {
    const editedLot = await this.LotModel.updateOne({ _id: LotID }, LotDto, { new: true });
    return editedLot;
  }

  async deleteLot(LotID: string): Promise<Lot> {
    const deletedLot = await this.LotModel.deleteOne({ _id: LotID });
    return deletedLot;
  }
